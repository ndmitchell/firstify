...
\begin{enumerate}
\item The transformation can \textit{reduce sharing}, causing the resulting program to be less efficient.  So our defunctionalisation method is not appropriate as a  stage in compilation.  But it works well used as a preliminary stage in program analysis, effectively making first-order analyses applicable to higher-order programs: examples include analyses for safe pattern-matching and for termination.
\item The transformation is \textit{not complete}.  In some programs there may be residual higher-order expressions.  However, the possible occurrences of such residual expressions can be characterised precisely, and mild restrictions on original programs guarantee first-order results.  In practice, our method is very often completely successful: for example defunctionalisation is complete for over 90\% of the nofib benchmark programs.
\end{enumerate}
...
\item We show how to apply our results to existing analysis tools, using GHC's strictness analyser and Agda's termination checker as examples (\S\ref{sec:strictness}).
\end{itemize}
...
\begin{definition}
A program is \textit{higher-order} if it contains expressions which create or use functional values. An expression creates a functional value if it is a partially-applied function or a lambda expression which does not contribute to the arity of a top-level function. An expression uses a functional value if it is an over-applied function or a general application.
\end{definition}
...
\noindent The simplification rules can now turn the program into a first-order variant, using the rule (case-con) from \S\ref{sec:simplification_detail}.
...
This section gives the rules, which are applied non-deterministically. Many programs require a combination of rules to be applied, for example, the initial |incList| example requires simplification and specialisation rules.
...
\noindent Here the function applications of |(.)|, |not| and |odd| are all partially applied. Three lambda expressions can be inserted by the (eta) rule:
...
\noindent Here |(expensive 1)| is computed at most once. Every application of the functional argument within |map| performs a single |(+)| operation. After applying the (let-lam) rule we get:
...
\noindent Now |expensive| is recomputed for every element in |xs|. We include this rule in our simplifier, focusing on functional value removal at the expense of sharing.
\end{example}
...
The final three expressions all \textit{evaluate to} a boxed lambda, but are not boxed lambdas.
\end{example}
...
For each application of a top-level function in which at least one argument has a subexpression which is a lambda, a specialised variant is created, and used where applicable. The process follows the same pattern as constructor specialisation \cite{spj:specconstr}, but applies where function arguments are lambda expressions, rather than known constructors. Examples of common functions whose applications can usually be made first-order by specialisation include |map|, |filter|, |foldr| and |foldl|.
...
The idea is to generate templates for all function calls which pass functional values. Given an expression |e| a template is generated if: (1) |e| is a function application; and (2) at least one of the sub-expressions of |e| is either a lambda or a boxed lambda (see \S\ref{sec:inlining}). In all cases, the template generated is simply |e|.
...
Specialisation is the only rule which introduces new function names. In order to translate an expression in the resultant program to an equivalent expression in the input program, it is sufficient to replace all generated function names with their associated template, supplying all the necessary variables.
...
After transformation, there will be no partial applications, and all lambda expressions will be unreachable (never be evaluated at runtime), provided:
...
The (eta) rule removes partial application, and at the end of the transformation, no further rules apply -- therefore there can be no partial application in the resultant program.
\end{lemma}
...
\begin{lemma}{The parent of a lambda expression must be one of: the root of a function definition; an argument to an application; a lambda expression; an argument to a constructor}
...
\begin{lemma}{The parent of a boxed lambda must be one of: a boxed lambda; the root of a function definition; an argument to an application; a lambda expression}

Using the definition of |isBox| from Figure \ref{fig:boxed_lambda} to ignore expressions which are themselves boxed lambdas, the only possible parents of a boxed lambda not mentioned in the lemma are the binding of a let, the subject of a case, and as an argument to a function. We remove the binding of a let with (bind-box) and the argument to a function with specialisation.
...
\begin{lemma}{A function whose body is a boxed lambda must be called from the argument of an application or inside a lambda}

An application of a function whose body is a boxed lambda is itself a boxed lambda. Therefore the restrictions on where a boxed lambda can reside apply to applications of these functions.
\end{lemma}

\begin{lemma}{All lambda expressions are unreachable}

The |main| function cannot be a boxed lambda, as that would be a functional value, and is disallowed by restrictions on |main|. There remain only three possible locations for lambda expressions or boxed lambdas:

\begin{enumerate}
\item As an argument to an application (|v hole|).
\item As the body of a lambda expression (|\v -> hole|).
\item Contained within a boxed lambda.
\end{enumerate}

None of these constructs binds a lambda expression to a variable, therefore in the first case |v| cannot be bound to a lambda. If |v| is not a lambda, then type checking means that |v| \textit{must} evaluate to $\bot{}$, and |hole| will never be evaluated. In the remaining two cases, the lambda or boxed lambda must ultimately be contained within an application whose variable evaluates to $\bot{}$ -- and therefore will not be evaluated.
\end{lemma}

\begin{lemma}{There are no partial applications and all lambda expressions are unreachable}

By combining the lemmas that there are no partial applications and that all lambda expressions are unreachable.
\end{lemma}
...
After applying the (eta) rule and performing these rewrites, any program is guaranteed to be first-order.
...
For each definition function, we associate a set of expressions $S$. After generating a template $t$, we only specialise with that template if $\forall s \in S \bullet \neg(s \unlhd t)$. After specialising an expression $e$ with template $t$, we add $t$ to the set $S$ associated with the function definition containing $e$. When we generate a new function from a template, we copy the $S$ associated with the function at the root of the template.
...
Of the 66 programs tested, 60 can be rewritten as first-order using the rules defined in \S\ref{sec:consequences}. When looking at the resultant programs, 3 contain lambda expressions, and all but 5 contain expressions which could use functional values.
...
In this section we show that our method can be used to improve the results of an existing analysis. Our method is already used by the Catch tool \cite{me:catch}, to allow a first-order pattern-match analysis to check higher-order programs. We now give examples of applying our method to existing strictness and termination analyses.
...
\begin{example}
The Agda compiler \cite{agda} checks that each function is terminating, using an analysis based on the Foetus checker \cite{abel:foetus}.
...
No doubt there are other ways in which the above analyses could be improved, by extending and reworking the analysis machinery itself. But a big advantage of adding a preliminary defunctionalisation stage is that it is modular: the analysis itself is treated as a black box. Reynolds style defunctionalisation would not have improved either analysis.
