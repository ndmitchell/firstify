...
\todo{think about variable names, perhaps have m/n range of expressions and x over variables? perhaps we only need one expression, so e range of expressions?}

\todo{be careful: function vs top-level defined function}
...
\todo{finish this section, citations etc.}

This section should "describe how to apply our results to an existing analysis tool, using GHC's strictness analyser as an example (\S\ref{sec:strictness})"

Don't want to arbitrarily do the steps in our transformation, many important things like let-positioning \cite{spj:letfloating} are very critical and don't want to be played around with.
