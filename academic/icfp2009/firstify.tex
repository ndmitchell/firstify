...
\begin{figure}
\ignore\begin{code}
data Expr  =  ELam v x               {-"\text{\hspace{1cm}lambda abstraction}"-}
           |  EFun f xs_             {-"\text{\hspace{1cm}function application}"-}
           |  ECon c xs_             {-"\text{\hspace{1cm}constructor application}"-}
           |  EApp x xs_             {-"\text{\hspace{1cm}general application}"-}
           |  EVar v                 {-"\text{\hspace{1cm}variable}"-}
           |  ELet v x y             {-"\text{\hspace{1cm}non-recursive let expression}"-}
           |  ECase x alts_          {-"\text{\hspace{1cm}case expression}"-}

data Alt   =  EAlt c vs_ x           {-"\text{\hspace{1cm}case alternative}"-}
...
\begin{definition}
A program which contains no internal lambda expressions and no partially-applied top-level functions is first-order.
\end{definition}
...
For example, the program |foo x = x 1| is \textit{not} higher-order, as at runtime it will not \textit{create} any functional values. The only way for |x| to be assigned as higher-order is for it to be $\bot{}$, for a value to be created and given to it, or for an external piece of code to call it.

\section{Our First-Order Reduction Method}
\label{sec:overview}

Our method works by combining a few separate and well-known transformations. Each transformation on its own is correct, and none introduces any additional data types. We divide our transformations up in to three groups:
...
Each transformation has the possibility of removing some functional values, but the key contribution of this paper is \textit{how they can be used together} -- including which restrictions are necessary. All the transformations are applied non-deterministically at any location they apply.
...
Simplification serves to group several simple transformations that most optimising compilers apply. Some of these steps have the ability to remove functional values; others simply ensure a normal form for other transformations. All the rules given in \S\ref{sec:simplification_detail}.
...
The transformations we have presented are not confluent, consider \todo{example here}.

However, we conjecture that the rules in each of the individual sections are confluent. In order to ensure a deterministic application of the rules we always favour rules first from the simplification stage, then the inlining stage, and finally the specialisation stage.
