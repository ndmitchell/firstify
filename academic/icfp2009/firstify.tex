...
\todo{discuss algebraic types}

\todo{be careful: function vs top-level defined function}

\todo{make very clear - non-recursive let's}

\todo{make more precise the type differences, what type info do we rely on}

\todo{give an intuition behind boxed lambdas}

\todo{more information about homeomorphic embedding and the intuition - were there alternatives available}

\todo{say more about why programs get smaller}

\todo{note that small program size probably implies faster analysis}

\todo{can you fuse the defunc and the analysis? interesting question for the conclusion}

\todo{mention dead code elimination somewhere}

\todo{mention that it is a pure lazy language}

\todo{emphasise that Reynold's + mine first will give a much simpler program}

\todo{say that the starting point in the results are the number of HO after initial desugaring, including type classes in to dictionaries etc}
...
\todo{we reuse template definitions indefinitely}
...
\todo{strengthen, say we do make programs first-order}
...
GHC uses demand analysis to figure out things. This is restricted to first-order values. For example, the following program is detected to be lazy in both its arguments:

\begin{code}
main :: Double -> Double -> Double -> Double
main x y = apply 10 (+x) y

apply :: Int -> (Double -> Double) -> Double -> Double
apply 0 f x = x
apply n f x = apply (n-1) f (f x)
\end{code}

But by generating a first-order variant of |main| and asking for its strictness information we get back that the argument |x| is strict and that the argument |y| is both strict and should be passed unboxed. In this particular case the gain is from specialisation giving a more efficient implementation, but in general it may also work. 
