...
\begin{enumerate}
\item The transformation can \textit{reduce sharing}, causing the resulting program to be less efficient and duplicate an arbitrary amount of work. Our method is still useful if the program is transformed, analysed, and then the results of the analysis are applied to the original program. For certain types of analysis (i.e. pattern-match checking and termination) this technique works well, for others (i.e. compilation) it does not.
\item Our algorithm is not complete -- it \textit{does not} always succeed in removing all functional values. However, in practice it is remarkably successful, removing all functional values from over 90\% of the nofib suite (see \S\ref{sec:results}). Even when it does not succeed, it usually reduces the number of functional values -- allowing a subsequent analysis to either use a poor approximation of functional values, or to use Reynolds' method afterwards, resulting in a simpler first-order program than Reynolds' method alone.
\end{enumerate}
...
In this section we show that our method can be used to improve the results of an existing analysis. As an example we choose GHC's demand analysis \cite{ghc}, which is responsible for determining which arguments to a function are strict.
...
While it is possible to directly strengthen the demand analysis in GHC, doing so is very dependent on the precise details of the analysis -- while a combination with our first-order reduction method treats the analysis as a black box. Another way to achieve more accurate results for GHC's demand analysis would be for the compiler to apply some of the rules from our method as part of its optimisation. However, some of our rules loose sharing, and there are many details which GHC pays careful attention to, such as where let expressions are located \cite{spj:letfloating}, which our rules do not take account of.
