...
Simplification serves to group several simple transformations that most optimising compilers apply. Some of these steps have the ability to remove functional values; others simply ensure a normal form for future transformations. \todo{cite where all these rules can be found, remove individual citations}
...
\noindent The simplification rule (let-lam) from \S\ref{sec:simplification_detail} lifts the lambda outside of the let expression.
...
We measured program size by taking the number of nodes in the abstract syntax tree. On average the size of the resultant program is smaller by 33\% (see Size in Table \ref{tab:results}). The decrease in program size is due to the elimination of dictionaries holding references to unnecessary code.
