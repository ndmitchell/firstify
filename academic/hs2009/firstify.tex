...
\conferenceinfo{Haskell Symposium 2009}{date, City.} %
\copyrightyear{2009} %
...
\begin{abstract}
\todo{The abstract is not as motivating as it could be, and misses the main goal of the work, which I have understood to be the analysis of higher-order programs by first-order analyses.}
...
\todo{Add something like in 4.1, namely "The simplification rules aim to move lambda expressions upwards, and introduce lambdas for partially applied functions."}
...
\todo{In section 3.4, the authors introduce some goals divided into two classes: essential and desirable. It would be interesting to have a further explanation of why they classified each goal in each category.}
...
This section gives a set of rules, which are applied non-deterministically \todo{line overflow}. Many programs require a combination of rules to be applied, for example, the initial |incList| example requires simplification and specialisation rules.
...
\todo{draw out app-app and fun-app as because of our AST}
...
The |isBox| function as presented may not terminate, but by simply keeping a list of followed functions, we can assume the result is |False| in any duplicate call. This modification does not change the result of any previously terminating evaluations. \todo{explain}
...
\todo{not clear how free variables are handled (in Example 9 instead of |\x->x| what if you had |\x-> x:xs| ?). It is not clear how the free variables interact with the templates.}
...
\todo{example has changed, not same as original}
...
\todo{The purpose of Section 6 is to classify any cases not covered.  In doing so we show that no obvious rule is missing and that all the rules are needed.}
...
\todo{not contributing to the arity of a function}
...
\todo{try and motivate this a bit better}
...
\todo{Cite: higher order functions considered unnecessary for higher-order programming. http://www-cse.ucsd.edu/~goguen/pps/utyop.ps}
