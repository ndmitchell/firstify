...
\paragraph{Restriction 2: Simplification Rules}
...
We apply the lambda rules from Figure \ref{fig:lambda_simplify}. As \ignore|(e - lam e)| occurs repeatedly we have factored it out as |l'|.
...
We can use the results of arity raising, which ensures that no function body is a lambda expression.
...
To deal with inlining, we need to work with lambda boxes, as defined by the function |isBox|, from Figure \ref{fig:boxed_lambda}. We first define the subset of expressions which are not lambda boxes:
...
We make use of the specialise rule, which removes all lambdas and boxed lambdas from the arguments of function applications:
...
Having applied all the rules, we now classify what the parent expressions of a lambda may be. A lambda may not be the root of a function body, because |lam| is not a production in |s|. If we show only those expressions which permit a lambda expression as a direct child, leaving \ignore|...| for expressions not containing lambda's, we are left with the following upper bound for the set of residual expressions:
...
\ignore\begin{code}
e = lam b &+ fun b b' &+ con b &+ app b' b &+ var &+ case_ b' b &+ let_ b' b
\end{code}
...
\subsection{Termination Bound (Bound)}
...
\subsection{Creating of Functional Values (HO Create)}
...
One torture test we have used during development is \ignore|show (1 :: Double)|. The underlying implementation of |show| on |Double| makes use of all the above features, including use of the |IO| Monad to manipulate arrays.

\subsection{Uses of Functional Values (HO Use)}
...
\begin{itemize}
\item We transform functions in an order determined by a topological sort.
...
\subsection{Program Size (Size)}

On average the size of the resultant program is smaller by 33\%. We measured program size by taking the number of nodes in the abstract syntax tree. The reason for the decrease in program size is due to eliminating dictionaries holding references to unnecessary code.
