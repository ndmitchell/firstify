...
let v = x in y
    => y[v / x]
    where x {-" is a boxed lambda (see \S?) "-}
\end{code}}
...
The |isBoxedLambda| check may not terminate, however we can ensure it does by keeping a list of followed functions, and assuming False for each one we have already seen. This does not alter the behaviour.

We also need to inline any boxed lambdas bound at let expressions, using the (bind-box) rule.



In order to remove functional values, it is useful to detect which expressions evaluate to functional values. If an expression |x| evaluates to a lambda expression, we can indicate this knowledge by wrapping the expression inside a lambda -- given certain restrictions for correctness.

If an expression evaluates to a constructor which contains a functional value within it, there is no equivalent way of expressing this easily. As a result we have introduced boxed lambdas, written |Box x|. The evaluation of |Box x| is identical to the evaluation of |x|. We only introduce a boxed lambda if the result of evaluating |x| is a constructor containing a functional value. We permit a functional value to reside under several constructors.

\begin{example}
\begin{code}
yes0 = Box [\x -> x]
yes1 = Box (Maybe [\x -> x])
yes2 = Box (let y = 1 in [\x -> x])
no0 = \x -> id x
no1 = [id (\x -> x)]
no2 = id [\x -> x]
\end{code}
