...
We have applied our method to tests drawn from the imaginary and spectral section of the nofib benchmark. We have worked with all 14 tests in the imaginary section, but have restricted our focus to 35 out of the 47 spectral tests. The reason for leaving 12 out is that they do not compile with Yhc, typically due to missing libraries or unimplemented base library functions. The programs that are excluded are not highly higher-order, and we suspect their results would be similar to the remaining ones.

Most analysis operations work on an abstracted view of the program. For example, the underlying IO mechanism of the Yhc standard libraries uses handles and pointers to read files -- while an analysis tool would probably give an abstraction for the |readFile| function. For the purposes of the firstification, we have worked on unmodified Yhc libraries, including all the low-level detail. Some particular places where higher-order functions are used:

\begin{itemize}
\item The monadic bind operation is higher-order.
\item The IO monad itself is implemented as a function.
\item Dictionaries are implemented as tuples of functions.
\item The Haskell |Show| framework uses continuation passing style extensively.
...
\textbf{Mb} is the maximum residency of the Firstify program at garbage collection time.

\smallskip\smallskip

\begin{tabular*}{\textwidth}{lrrrrrrlll}
...
x2n1            & 4 & 345 & 0 & 385 & 2 & -57 \\
                & \multicolumn{7}{c} ...Plus 35 tests from the spectral suite... \\
Minimum         & 2 & 60 & 0 & 46 & 0 & -78 \\
Maximum         & 14 & 437 & 1 & 449 & 100 & 15 \\
Average         & 4.80 & 237.88 & 0.06 & 202.20 & 3.55 & -32.50 \\
\hline
\end{tabular*}
\end{table*}

All of these factors combine, and a particular torture test is |show (1 :: Double)|. The results are presented in Table \ref{tbl:results}. We have given detailed results for the 14 imaginary benchmarks, and a min/max/average for the spectral suite. The column

The only programs which don't firstify are:
...
Because this code is run within the IO monad, the return is a functional value, and is passed to the primitive seq.

Program: constraints, undefined is applied to the lambda. In pretty it is more directly an error call.

Program: pretty

All as a result of calling foldr1, making it return a function, and applying the function.

combine xs = foldr1 f nil xs

And then later calling combine xs
...
If we wanted this, we would have to treat error a lambda, or use the type information.
...
George Charles Nelan Jr. Firstification. PhD thesis, Arizona State University, Arizona, USA, December 1991.

PDF] An Introduction to Program Specialisation by Type Inference - all 6 versions »
J Hughes - Functional Programming, 1996 - repository.readscheme.org

41. D.H.D. Warren. Higher-order extensions to Prolog: Are they needed? In Machine Intelligence 10, pages 441–454,
1982.

8. S. Antoy and A. Tolmach. Typed higher-order narrowing without higher-order strategies. In 4th Fuji International
Symposium on Functional and Logic Programming (FLOPS’99), volume 1722, pages 335–350, Tsukuba, Japan, 11
1999. Springer LNCS

[CITATION] Implementation, Validation and Performance Analysis of a Firstification Technique
GS Banavar - 1991 - Arizona State University

[CITATION] Firstification
GC Nelan - 1991 - Arizona State University

A higher-order removal method - all 5 versions »
WN Chin, J Darlington - Higher-Order and Symbolic Computation, 1996 - Springer
Page 1. LISP AND SYMBOLIC COMPUTATION, 9, 287-322 (t996) @ 1996 Kluwer Academic
Publishers, Boston. Manufactured in The Netherlands. ...

W.N. Chin and J Darlington, A Higher-Order Removal Method,  Journal of Lisp and Symbolic Computation , Vol 9, 287-322, 1996.

 A higher-order removal method
Journal	LISP and Symbolic Computation
Publisher	Springer Netherlands
ISSN	0892-4635
Issue	Volume 9, Number 4 / December, 1996
DOI	10.1007/BF01806315
Pages	287-322
SpringerLink Date	Wednesday, June 22, 2005

@inproceedings{964009,
...
[PS] Defunctionalization of typed programs - all 5 versions »
JM Bell, J Hook - Oregon Graduate Institute School of Science Engineering, 1994 - cse.ogi.edu


Type-driven defunctionalization - all 6 versions »
JM Bell, F Bellegarde, J Hook - ACM SIGPLAN Notices, 1997 - portal.acm.org


Rabbit: A Compiler for Scheme - all 2 versions »
GL Steele Jr - 1978 - portal.acm.org
